...
const services = [
  ...
  { id: 'video', title: 'captação e edição de vídeo', description: 'reels/tiktoks para lançamentos, divulgação de shows, clipes e material dinâmico para redes sociais.', note: 'conteúdo para divulgar o que a banda está fazendo.', media: ['video-cover-1.webp', 'video-cover-2.webp', 'video-cover-3.webp'] },
...
function BrandMark({ onClick }) { return <a className="brand" href="#inicio" onClick={onClick} aria-label="isabella monteiro, início"><img className="brand-icon" src={image('isabella-mark.png')} alt="" aria-hidden="true" /><span className="brand-name"><span>isabella</span><span>monteiro</span></span></a> }
...
function Header() {
  ...
  return <header className={`site-header${open ? ' menu-is-open' : ''}`}><div className="scroll-progress" aria-hidden="true" /><BrandMark onClick={close} /><nav id="site-nav" className={`site-nav${open ? ' is-open' : ''}`} aria-label="navegação principal"><a href="#sobre" onClick={close}>quem sou</a><a href="#servicos" onClick={close}>serviços</a><a href="#trabalhos" onClick={close}>trabalhos</a><a href="#contato" onClick={close}>contato</a></nav><button className="menu-toggle" type="button" aria-controls="site-nav" aria-expanded={open} onClick={() => setOpen((value) => !value)}>{open ? 'fechar' : 'menu'}</button></header>
}

function Hero() { return <section className="hero hero-poster" id="inicio"><div className="hero-copy"><h1><span>profissionalize</span> <span>a comunicação</span> <span>da sua banda.</span></h1><div className="hero-meta"><p>músicos não precisam virar criadores de conteúdo. quem entende do assunto faz isso por você.</p><p>curitiba / brasil<br />comunicação para bandas independentes</p></div><WhatsAppLink>vamos falar sobre sua banda</WhatsAppLink></div><figure className="hero-visual hero-collage"><div className="hero-paper" aria-hidden="true" /><span className="hero-poster-photo"><img src={image('show-01.webp')} alt="público e banda durante show fotografado por isabella monteiro" fetchPriority="high" /></span><span className="hero-poster-photo"><img src={image('show-03.webp')} alt="banda tocando em um show fotografado por isabella monteiro" loading="eager" /></span><span className="hero-poster-photo"><img src={image('show-06.webp')} alt="apresentação ao vivo fotografada por isabella monteiro" loading="eager" /></span></figure></section> }
